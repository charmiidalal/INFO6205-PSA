 ...
 */
public class UF_HWQUPC_ALTERNATE implements UF {
    /**
     ...
     */
    public UF_HWQUPC_ALTERNATE(int n, boolean pathCompression) {
        count = n;
        parent = new int[n];
        height = new int[n];
        for (int i = 0; i < n; i++) {
     ...
     * @throws IllegalArgumentException if {@code n < 0}
     */
    public UF_HWQUPC_ALTERNATE(int n) {
        this(n, true);
    }
    ...
    public int find(int p) {
        ...
        if (this.pathCompression) {
            int currNode = root;
            while (this.parent[currNode] != currNode) {
                currNode = this.parent[currNode];
            }
            this.parent[root] = currNode;
        } else {
    ...
    private void updateParent(int p, int x) {
        this.parent[p] = this.parent[x];
    }

    private void updateHeight(int p, int x) {
        this.height[p] += this.height[x];
    }
